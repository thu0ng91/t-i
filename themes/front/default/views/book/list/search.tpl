	...
	{foreach from=$list item=item}
		...
		<div class="imgbox">
			...
			<img width="90" height="118" src="{$item->coverImageUrl}"	alt="{$item->title}" title="{$item->title}" />
			 <span class="txt_bg">
			{if $item->flag == Yii::app()->controller->module['front']['flagstatus']}连载中 {else}已完结 {$item->wordcount}万{/if}
			</span>
			</a>
		</div>
		<dl class="info">
			<dt>
				<a class="sub_link" href="{novel_book_link id=$item->id pinyin=$item->pinyin}" target="_blank">{$item->title|trim|truncate:10:"...":true}</a>
			</dt>
			...
			<dd>更新到：
				<span>
					<a class="more" href="{novel_chapter_link bookid=$item->id id=$item->lastchapterid pinyin=$item->pinyin}" target='_blank'>
						 {$item->lastchaptertitle|trim|truncate:17:"...":true}</a>
				</span>
