 ...
 * @subpackage Compiler
 */
class Smarty_Internal_Compile_Novel_notice extends Smarty_Internal_Book_Compilebase
{
...
//        $this->openTag($compiler, 'foreach', array('foreach', $compiler->nocache, $item, $key));
        $this->openTag($compiler, 'novel_notice');
...
//        $usesPropTotal = $usesSmartyTotal || $usesSmartyShow || $usesPropShow || $usesPropLast || strpos($tpl->source->content, $ItemVarName . 'total') !== false;
        ...
        $output .= " \$_data = _fw_get_book_notice_list(" . $varParams. ");\n";
//        if ($key != null) {
...
class Smarty_Internal_Compile_Novel_noticeclose extends Smarty_Internal_CompileBase
{
    ...
    public function compile($args, $compiler, $parameter)
    {
        ...
        list($openTag) = $this->closeTag($compiler, array('novel_notice'));
