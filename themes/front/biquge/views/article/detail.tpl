<div class="content_read">
    ...
    <div class="box_con">
        ...
        <div class="bookname">
            <h1>{$chapter->title}</h1>
            <div class="bottem1">
                <a onclick="showpop('{$this->createUrl("book/like", ["id" => $chapter->book->id])}');" href="javascript:;">投推荐票</a>{if isset($prevChapter)}<a href="{novel_chapter_link id=$prevChapter->id}">上一章</a> ←{/if}  <a href="{novel_book_link id=$chapter->book->id}">章节目录</a> {if isset($nextChapter)}→ <a href="{novel_chapter_link id=$nextChapter->id}">下一章</a>{/if} <a onclick="showpop('{$this->createUrl("user/addFavourite", ["id" => $chapter->book->id])}');" href="javascript:;">加入书签</a>
            </div>
